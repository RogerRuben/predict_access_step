"""
stage1_deep.py — 累积序数 + 4-Class 双头版 Final
  1. Ordinal head: q1/q2/q3 (风险概率, 给 Stage 2/3)
  2. Classifier head: 4-class softmax (分类性能, 给审稿人)
  3. Delta margin bias: 保底厚度防中间类塌陷
  4. Focal gamma: q1=0, q2=2, q3=2
  5. τ 搜索: 加 s2/s3 recall 下界约束
  6. Fused distribution: ordinal + classifier 融合
"""
...
# 融合系数
ETA_FUSE = 0.35  # p_fuse = eta * p_ord + (1-eta) * p_cls
...
class MonotonicOrdinalHead(nn.Module):
    def __init__(self, fusion_dim, hidden_dim, min_margin=0.3):
        super().__init__()
    ...
    def forward(self, fused):
        ...
        # ★ softplus + margin bias：保底厚度防中间类塌陷
...
class DualHeadOrdinalWDRNet(nn.Module):
    def __init__(self, dense_dim, hidden_dim=64, num_layers=2, min_margin=0.3):
        ...
        # 双头
        self.ordinal_head = MonotonicOrdinalHead(fusion_dim, hidden_dim, min_margin)
    ...
    def forward(self, x, lengths, gate_info):
        ...
        lq1, lq2, lq3 = self.ordinal_head(fused)
        cls_logits = self.classifier_head(fused)  # (B, T, 4)

        return lq1, lq2, lq3, cls_logits

    def get_all_probs(self, lq1, lq2, lq3, cls_logits, eta=ETA_FUSE):
        """返回 ordinal / classifier / fused 三套概率"""
...
class HybridOrdinalClassificationLoss(nn.Module):
    """
    L = L_ord + lambda_cls * L_cls + current_lambda_cons * L_cons

    说明:
      - 保留 p_ord.detach()，保证 ordinal branch 继续做风险教师
      - 通过 current_lambda_cons 动态调节 consistency 强度
      - 允许 warm-up 阶段先把 classifier 头练起来
    """

    def __init__(self, pw_q1, pw_q2, pw_q3,
                 cls_alpha,
                 gamma_q1=0.0, gamma_q2=2.0, gamma_q3=2.0,
                 gamma_cls=2.0,
                 lambda_cls=1.0, lambda_cons=0.1,
                 ignore_index=-1):
        super().__init__()
        ...
        # ★ 当前 epoch 实际使用的 consistency 权重（可被训练循环动态修改）
    ...
    def set_consistency_weight(self, value: float):
        """给训练循环调用，动态调整 consistency loss 权重。"""
        self.current_lambda_cons = float(value)
    ...
    def _focal_ce(self, logits, targets, alpha, gamma):
        ...
        focal = (1 - pt).pow(gamma)
        return (alpha[targets] * focal * ce).mean()

    def forward(self, lq1, lq2, lq3, cls_logits, targets):
        B, T = targets.shape
        ...
        l_cons = F.mse_loss(p_cls, p_ord.detach())

        total = l_ord + self.lambda_cls * l_cls + self.current_lambda_cons * l_cons

        return total, l_ord, l_cls, l_cons, l_q1, l_q2


# ================================================================
# 5. Quantile Decision（不变）
# ================================================================
...
def estimate_weights(train_shards, max_shards=5):
    """估计 ordinal pw + classifier alpha"""
    ...
    # Classifier alpha (inverse frequency, sqrt smoothed)
    total = counts.sum()
...
def evaluate_on_shards(model, shard_infos, device, criterion,
    ...
    all_ord_preds, all_cls_preds, all_fuse_preds = [], [], []
        ...
        with torch.no_grad():
            for X_b, y_b, lens_b in loader:
                ...
                with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
                    lq1, lq2, lq3, cls_logits = model(X_b, lens_b, gate_b)
                    loss, l_ord, l_cls, l_cons, _, _ = criterion(
                        lq1, lq2, lq3, cls_logits, y_b
                    )

                # ★ 修正 1：loss 只在 autocast 块外累加一次
                ...
                # ★ 修正 2：只调用一次 get_all_probs，不再有残留废弃写法
                p_ord, p_cls, p_fuse, q1, q2, q3 = model.get_all_probs(
                    lq1, lq2, lq3, cls_logits
                )

                # ★ 修正 3：q1/q2/q3 统一 .float() 进决策函数
                ord_pred, should_reject, _ = ordinal_quantile_decision(
                    q1.float(), q2.float(), q3.float(), lens_b, tau=tau
                )

                cls_pred  = p_cls.argmax(dim=-1)
                fuse_pred = p_fuse.argmax(dim=-1)

                for b in range(len(lens_b)):
                    L = lens_b[b].item()
                    all_ord_preds.extend(ord_pred[b, :L].cpu().numpy())
                    all_cls_preds.extend(cls_pred[b, :L].cpu().numpy())
                    all_fuse_preds.extend(fuse_pred[b, :L].cpu().numpy())
    ...
    fuse_p  = np.array(all_fuse_preds)[m]
    ...
    r_fuse = metrics(fuse_p, "Fused")

    if verbose:
        for r in [r_ord, r_cls, r_fuse]:
            ...
            log.info(
                f"Acc={r['acc']:.4f} | mF1={r['mf1']:.4f} | "
                f"DMR={r['dmr']:.4f} | s4u={r['s4u']:.4f}"
            )
            log.info(
                f"Recall: [{', '.join(f's{i+1}={rv:.4f}' for i, rv in enumerate(r['rec']))}]"
            )
    ...
    return {
        ...
        "accuracy":        r_ord["acc"],
        "macro_f1":        r_ord["mf1"],
        "dangerous_miss":  r_ord["dmr"],
        "s4_underestimate":r_ord["s4u"],
        "recall_per_class":r_ord["rec"],
        "reject_rate":     float(rej.mean()),
        "cls_macro_f1":    r_cls["mf1"],
        "cls_recall":      r_cls["rec"],
        "fuse_macro_f1":   r_fuse["mf1"],
        "fuse_recall":     r_fuse["rec"],
    }

# ================================================================
# 8. τ 搜索（加 s2/s3 recall 下界）
# ================================================================
...
def search_best_tau(model, val_shards, device, criterion,
    ...
    log.info("\n" + "=" * 60)
    log.info("TAU SEARCH (with s2/s3 recall constraints)")
    log.info("=" * 60)
    ...
    while True:
        ...
        with torch.no_grad():
            for X_b, y_b, lens_b in loader:
                ...
                gate_b = X_b[:, :, GATE_INDICES]
                with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
                    lq1, lq2, lq3, _ = model(X_b, lens_b, gate_b)
    ...
    probs = np.stack([1.0-q1n, q1n-q2n, q2n-q3n, q3n], axis=-1)
    ...
    if len(safe) > 0:
        ...
        if len(safe2) > 0:
            best_row = safe2.loc[safe2["macro_f1"].idxmax()]
        else:
            best_row = df.loc[df["DMR"].idxmin()]
...
def train_wrc_from_shards(
    ...
    log.info("=" * 70)
    log.info("Dual-Head Ordinal+Classifier WDR")
    log.info("=" * 70)
    ...
    model = DualHeadOrdinalWDRNet(len(feature_cols), hidden_dim, num_layers).to(device)
    log.info(f"Params: {sum(p.numel() for p in model.parameters()):,}")
    log_gpu_memory("Init ")

    criterion = HybridOrdinalClassificationLoss(
        pw_q1=pw_q1, pw_q2=pw_q2, pw_q3=pw_q3, cls_alpha=cls_alpha,
        gamma_q1=0.0, gamma_q2=2.0, gamma_q3=2.0, gamma_cls=2.0,
        lambda_cls=1.0, lambda_cons=0.1,
    ).to(device)
    ...
    tb_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "runs", f"dualhead_{ts}")
    writer = SummaryWriter(log_dir=tb_dir)

    latest_ckpt = os.path.join(CHECKPOINT_DIR, "dualhead_latest.pt")
    best_ckpt = os.path.join(CHECKPOINT_DIR, "dualhead_best.pt")
    ...
    for epoch in range(start_epoch, epochs):
        model.train()
        t_loss = t_ord = t_cls = t_cons = 0.0

        # ★ consistency warm-up
        # 前 2 个 epoch 先让 classifier 头自由学习，不受 ordinal 蒸馏约束
        if epoch < 2:
            criterion.set_consistency_weight(0.0)
        else:
            criterion.set_consistency_weight(0.05)  # 比原来 0.1 更温和

        log.info(f"  consistency_lambda = {criterion.current_lambda_cons:.3f}")
        nb = 0; epoch_start = time.time()
        ...
        while True:
            ...
            for bi, (X, y, lens) in enumerate(pbar):
                ...
                with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
                    lq1, lq2, lq3, cls_logits = model(X, lens, gate)
                    loss, l_ord, l_cls, l_cons, _, _ = criterion(lq1, lq2, lq3, cls_logits, y)
                ...
                if device.type == "cuda" and bi % 20 == 0:
                    ...
                    pbar.set_postfix(loss=f"{t_loss/nb:.4f}", ord=f"{t_ord/nb:.3f}",
                                     cls=f"{t_cls/nb:.3f}", gpu=f"{alloc:.1f}G")
            ...
            if sc % 30 == 0:
                ...
                log.info(f"  [E{epoch+1} S{sc}/{len(shards)}] loss={t_loss/nb:.4f} "
                         f"(ord={t_ord/nb:.3f} cls={t_cls/nb:.3f} cons={t_cons/nb:.3f}) | "
        ...
        log.info(f"  Loss={a(t_loss):.4f} (ord={a(t_ord):.3f} cls={a(t_cls):.3f} cons={a(t_cons):.3f})")
        ...
        log.info(
            f"  Val total={val['loss']:.4f} | "
            f"ord={val['loss_ord']:.4f} cls={val['loss_cls']:.4f} cons={val['loss_cons']:.4f}"
        )
        log.info(
            f"  Ord  mF1={val['macro_f1']:.4f} | "
            f"Cls  mF1={val['cls_macro_f1']:.4f} | "
            f"Fuse mF1={val['fuse_macro_f1']:.4f} | "
            f"DMR={val['dangerous_miss']:.4f}"
        )
        log.info(
            f"  Ord  Recall:  [{', '.join(f's{i + 1}={r:.3f}' for i, r in enumerate(val['recall_per_class']))}]"
        )
        log.info(
            f"  Cls  Recall:  [{', '.join(f's{i + 1}={r:.3f}' for i, r in enumerate(val['cls_recall']))}]"
        )
        log.info(
            f"  Fuse Recall:  [{', '.join(f's{i + 1}={r:.3f}' for i, r in enumerate(val['fuse_recall']))}]"
        )
        writer.add_scalar("Loss/train", a(t_loss), epoch)
        writer.add_scalar("M/ord_mf1", val["macro_f1"], epoch)
        writer.add_scalar("M/cls_mf1", val["cls_macro_f1"], epoch)
        writer.add_scalar("M/fuse_mf1", val["fuse_macro_f1"], epoch)
        ...
        # best checkpoint 用 fused macro_f1（兼顾分类和安全）
        imp = (val["dangerous_miss"] < best_dmr or
               (abs(val["dangerous_miss"] - best_dmr) < 1e-8 and val["fuse_macro_f1"] > best_mf1))
        if imp:
            best_dmr = val["dangerous_miss"]
            best_mf1 = val["fuse_macro_f1"]
            best_state = {k: v.cpu().clone() for k, v in model.state_dict().items()}
            save_training_checkpoint(model, optimizer, scheduler, scaler,
                                     epoch, best_dmr, best_mf1, best_ckpt)
            no_improve = 0
            log.info(f"  ★ BEST: DMR={best_dmr:.4f} | fuse_mF1={best_mf1:.4f}")
        else:
            no_improve += 1
    ...
    best_tau, tau_df = search_best_tau(model, val_shards, device, criterion, batch_size*2, max_seq_len)
...
def save_wrc_model(model, tag="dualhead_v1"):
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = os.path.join(MODEL_DIR, f"stage1_dualhead_{tag}_{ts}.pt")
    torch.save({
        "model_state": model.state_dict(),
        "dense_dim": model.dense_dim, "hidden_dim": model.hidden_dim,
        "num_layers": model.num_layers,
        "model_type": "dual_head_ordinal_classifier",
...
def load_wrc_model(path, device=None):
    ...
    model = DualHeadOrdinalWDRNet(
        d["dense_dim"], d["hidden_dim"], d["num_layers"]
    ).to(device)
    model.load_state_dict(d["model_state"])
    model.eval()
    if "best_tau" in d: model.best_tau.fill_(d["best_tau"])
...
def predict_proba_wrc(model, df, max_seq_len=WRC_MAX_SEQ_LEN,
    ...
    for col in feature_cols:
        if col in PERIODIC_FEATURES: x[col] = x[col].clip(-1.0, 1.0)
        else:
    ...
    for start in tqdm(range(0, len(keys), batch_size),
                      desc="[DualHead predict]", dynamic_ncols=True, unit="b"):
        bk = keys[start:start + batch_size]
        seqs, ri, lens = [], [], []
        for key in bk:
            grp = grouped.get_group(key)
            idxs = grp["_row_idx"].values; feats = scaled[idxs]
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
                lq1, lq2, lq3, cls_logits = model(sp, lt, gt)
                p_ord, p_cls, p_fuse, q1_t, q2_t, q3_t = model.get_all_probs(
                    lq1, lq2, lq3, cls_logits
                )

                pred_cls_t, should_reject, _ = ordinal_quantile_decision(
                    q1_t.float(), q2_t.float(), q3_t.float(), lt, tau=tau
                )

                fuse_np = p_fuse.cpu().numpy()
        ...
        for b, (L, idxs) in enumerate(zip(lens, ri)):
            pred_proba[idxs] = fuse_np[b, :L]           # ★ fused 概率
            pred_q2[idxs] = q2_np[b, :L]                # ordinal q2
            pred_q3[idxs] = q3_np[b, :L]                # ordinal q3
