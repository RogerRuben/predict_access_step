...
def train_wrc_from_shards(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        nb = 0
        epoch_start_time = time.time()  # ★ 需要 import time
        ...
        while True:
            ...
            for bi, (X, y, lens) in enumerate(pbar):
                ...
                t_cong += float(l_cong.item())
                t_severe += float(l_sev.item())
                t_mild += float(l_mild.item())
                nb += 1
            ...
            # ★ 每 30 个 shard 打印一次中间摘要
            if sc % 30 == 0:
                elapsed = time.time() - epoch_start_time
                speed = nb / max(elapsed, 1)
                log.info(
                    f"  [E{epoch + 1} S{sc}/{len(shards)}] "
                    f"loss={t_loss / nb:.4f} "
                    f"(cong={t_cong / nb:.3f} sev={t_severe / nb:.3f} mild={t_mild / nb:.3f}) | "
                    f"speed={speed:.1f}b/s | "
        ...
        log.info(f"  Total batches:   {nb:,}")
        log.info(f"  Total time:      {epoch_time:.0f}s ({epoch_time / 60:.1f}min)")
        log.info(f"  Speed:           {nb / max(epoch_time, 1):.1f} b/s")
        log.info(f"  Train loss:      {a(t_loss):.4f}")
        log.info(f"    cong loss:     {a(t_cong):.4f}")
        log.info(f"    severe loss:   {a(t_severe):.4f}")
        log.info(f"    mild loss:     {a(t_mild):.4f}")
        log.info(f"  Global step:     {global_step}")
        log.info(f"  Learning rate:   {optimizer.param_groups[0]['lr']:.1e}")
        ...
        val = evaluate_on_shards(
            ...
            batch_size * 2, max_seq_len, verbose=False,
        )

        val_time = time.time() - val_start

        log.info(f"  Val loss:        {val['loss']:.4f}")
        log.info(f"  Accuracy:        {val['accuracy']:.4f}")
        log.info(f"  Macro F1:        {val['macro_f1']:.4f}")
        log.info(f"  DMR:             {val['dangerous_miss']:.4f}")
        log.info(f"  s4 underestimate:{val['s4_underestimate']:.4f}")
        log.info(f"  Reject rate:     {val['reject_rate']:.4f}")
        log.info(f"  Recall: [{', '.join(f's{i + 1}={r:.3f}' for i, r in enumerate(val['recall_per_class']))}]")
        log.info(f"  Val time:        {val_time:.0f}s")

        clr = optimizer.param_groups[0]["lr"]

        # TensorBoard
        writer.add_scalar("Loss/train", a(t_loss), epoch)
        writer.add_scalar("Loss/cong", a(t_cong), epoch)
        writer.add_scalar("Loss/sev", a(t_severe), epoch)
        writer.add_scalar("Loss/mild", a(t_mild), epoch)
        ...
        writer.add_scalar("M/reject", val["reject_rate"], epoch)
        ...
        imp = (
                val["dangerous_miss"] < best_dmr
                or (abs(val["dangerous_miss"] - best_dmr) < 1e-8 and val["macro_f1"] > best_mf1)
        )
        if imp:
            ...
            no_improve = 0
            log.info(f"  ★ BEST checkpoint: DMR={best_dmr:.4f} | macro_f1={best_mf1:.4f}")
        else:
            no_improve += 1
