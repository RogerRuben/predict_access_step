...
from config import (
    ...
    TRAIN_SAMPLE_RATE, BATCH_SIZE_ORDERS, MODEL_DIR, CACHE_DIR,STAGE1_MODEL,
)
...
def _run_stage1_train(topo):
    ...
    # if STAGE1_MODEL == "wrc":
    #     # ----------------------------------------------------------------
    #     # WRC 分支：收集 DataFrame，不转 numpy
    #     # ----------------------------------------------------------------
    #     from stage1_deep import train_wrc_model, save_wrc_model
    #
    #     train_feat_dfs = []
    #     for day in TRAIN_DAYS:
    #         try:
    #             log.info(f"Extracting day {day} for WRC ...")
    #             head, link, cross = load_day(day)
    #
    #             unique_orders = link["order_id"].unique()
    #             for i in range(0, len(unique_orders), BATCH_SIZE_ORDERS):
    ...
    #             del head, link, cross
    #             gc.collect()
    #             log.info(f"  Day {day} collected.")
    #
    #         except FileNotFoundError:
    #             log.warning(f"Day {day} missing, skipped.")
    #
    #     if not train_feat_dfs:
    #         raise RuntimeError("No training data for WRC")
    #
    #     train_feat_all = pd.concat(train_feat_dfs, ignore_index=True)
    #     del train_feat_dfs
    #     gc.collect()
    #
    #     log.info(f"Total WRC training rows: {len(train_feat_all):,}")
    #
    #     model = train_wrc_model(
    #         train_feat_all,
    ...
    if STAGE1_MODEL == "wrc":
        ...
        model_path = save_wrc_model(model, tag="shard_v1")
        return model, model_path
    else:
        X_parts, y_parts = [], []
        for day in TRAIN_DAYS:
